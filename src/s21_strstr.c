...
char *s21_strstr(const char *haystack, const char *needle) {
  ...
  for (; *haystack; haystack++) {
    // initialization of constant pointers to the first elements of the needle
    // and haystack
    const char *h = haystack;
    const char *n = needle;

    // the characters are compared until the needle runs out or there is no
    // discrepancy
    while (*h && *n && (*h == *n)) {
      h++;
      n++;
    ...
    // if the end of the needle has been reached, then the first occurrence of
